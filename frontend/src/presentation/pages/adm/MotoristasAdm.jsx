...
const formatDate = (iso) => {
  if (!iso) return '';
  const str = String(iso).substring(0, 10);
  if (str.includes('-')) {
...
export const MotoristasAdm = () => {
  ...
  const handleSalvar = async (e) => {
    ...
    if (editingId) {
      ...
      try {
        const uniqueEmailFallback = `${formNome.toLowerCase().split(' ')[0]}.${Date.now()}@siti.com`;
        await httpClient.post('/admin/drivers', {
          name: formNome,
          email: formEmail || uniqueEmailFallback,
          cnh: (formCnh || '00000000000').replace(/\D/g, ''),
          category: formCategoria,
          phone: formTelefone,
          birthDate: parseDate(formNascimento || '01/01/1985'),
                  ...
                  <tbody>
                    {motoristas.map((m) => (
                      <tr key={m.id}>
                        ...
                        <td className="fw-monospace">{m.cpf || 'Não informado'}</td>
