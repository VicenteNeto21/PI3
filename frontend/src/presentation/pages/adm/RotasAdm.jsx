...
export const RotasAdm = () => {
  ...
  const [enqueteAberta, setEnqueteAberta] = useState(() => {
  ...
  const [rotas, setRotas] = useState([
    {
      ...
      origem: 'Rua São José',
      destino: 'Campus UFC - Benfica',
      paradas: 5,
      motorista: 'Roberto Silva',
      veiculo: 'ABC-1234 (Volare W9)',
      status: 'Ativa',
      stopsList: ['Rua São José', 'Av. Universitária', 'Praça da Matriz', 'Praça Gentil Cardoso', 'Campus UFC - Benfica']
    },
    {
      id: 2,
  ...
  const fetchRoutes = async () => {
    try {
      logger.info('ROTAS ADM', '🗺️ Buscando rotas e paradas no banco da API (/admin/routes e /admin/stops)...');
      const [resRoutes, resStops] = await Promise.all([
        httpClient.get('/admin/routes').catch(() => null),
        httpClient.get('/admin/stops').catch(() => null)
      ]);

      let cleanStops = ['Rua São José', 'Av. Universitária', 'Praça da Matriz', 'Praça Gentil Cardoso', 'Campus UFC - Benfica'];
      if (resStops && Array.isArray(resStops) && resStops.length > 0) {
        const dbStops = resStops.map(s => s.street).filter(Boolean);
        if (dbStops.length > 0) {
          cleanStops = dbStops;
        }
      }

      if (resRoutes && Array.isArray(resRoutes) && resRoutes.length > 0) {
        logger.success('ROTAS ADM', `📍 ${resRoutes.length} linha(s) carregada(s) do banco de dados da API!`);
        const mapped = resRoutes.map((r, idx) => {
          const stopsArray = r.stops ? String(r.stops).split(',').map(s => s.trim()).filter(Boolean) : cleanStops;
          ...
          return {
            ...
            origem: origem,
            destino: destino,
            paradas: stopsArray.length > 0 ? stopsArray.length : cleanStops.length,
            motorista: r.motorista || 'Roberto Silva',
            veiculo: r.veiculo || 'ABC-1234 (Volare W9)',
            status: r.status || 'Ativa',
            stopsList: stopsArray.length > 0 ? stopsArray : cleanStops
          };
        });
        setRotas(mapped);
        if (mapped.length > 0) {
          ...
          setEmbarqueStops(firstRoute.stopsList || cleanStops);
          setDesembarqueStops([...(firstRoute.stopsList || cleanStops)].reverse());
        }
      } else {
        logger.warn('ROTAS ADM', '⚠️ Nenhuma rota encontrada na API, mantendo rotas padrão com paradas limpas.');
        setEmbarqueStops(cleanStops);
        setDesembarqueStops([...cleanStops].reverse());
      }
    } catch (error) {
  ...
  // Ações das Paradas de Embarque
  const handleAddEmbarque = async (e) => {
    e.preventDefault();
    if (!newEmbarque.trim()) return;
    const cleanName = newEmbarque.trim();
    try {
      await httpClient.post('/admin/stops', { street: cleanName, neighborhood: 'Centro', buildingNumber: 's/n' });
    } catch (err) {
      logger.error('ROTAS ADM', 'Erro ao salvar parada na API', err);
    }
    const newStops = [...embarqueStops, cleanName];
  ...
  const handleAddDesembarque = async (e) => {
    e.preventDefault();
    if (!newDesembarque.trim()) return;
    const cleanName = newDesembarque.trim();
    try {
      await httpClient.post('/admin/stops', { street: cleanName, neighborhood: 'Centro', buildingNumber: 's/n' });
    } catch (err) {
      logger.error('ROTAS ADM', 'Erro ao salvar parada na API', err);
    }
    const newStops = [...desembarqueStops, cleanName];
  ...
  // Ação de Encerrar/Ocultar Enquete
  const handleEncerrarEnquete = async () => {
    try {
      await httpClient.put('/api/polls/1/close').catch(() => null);
    } catch {
      // fallback
    }
    localStorage.setItem('siti_enquete_aberta', 'false');
    setEnqueteAberta(false);
    setAlertMsg('Enquete ocultada/fechada. Os alunos aguardarão a liberação da enquete.');
  ...
  const handleDisponibilizarEnquete = async () => {
    try {
      ...
      // 4. Publicar enquete e definir como disponível
      await httpClient.put('/api/polls/1/publish').catch(() => null);
    } catch {
      // fallback offline
    }
    localStorage.setItem('siti_enquete_aberta', 'true');
    setEnqueteAberta(true);
    setAlertMsg('Enquete de presença aberta e disponibilizada aos alunos com sucesso!');
    setTimeout(() => setAlertMsg(''), 5000);
  };
    ...
    <div className="rotas-adm-container w-100 h-100 d-flex flex-column">
      ...
      {view === 'figma' && (
        <div className="flex-grow-1 d-flex flex-column">
          {/* Card de Status da Enquete para Alunos */}
          <div className="d-flex justify-content-between align-items-center p-3 mb-4 rounded-4 shadow-sm border" style={{ backgroundColor: enqueteAberta ? '#F0FDF4' : '#FFFBEB', borderColor: enqueteAberta ? '#BBF7D0' : '#FEF08A' }}>
            <div className="d-flex align-items-center gap-3">
              <span style={{ fontSize: '1.6rem' }}>{enqueteAberta ? '🟢' : '🔒'}</span>
              <div>
                <h6 className="fw-bold m-0" style={{ color: enqueteAberta ? '#166534' : '#854D0E' }}>
                  Status da Enquete para Alunos: {enqueteAberta ? 'ABERTA / DISPONÍVEL' : 'FECHADA (EM PREPARAÇÃO)'}
                </h6>
                <small className="text-muted">
                  {enqueteAberta 
                    ? 'Os alunos conseguem acessar e votar nos locais de embarque, desembarque e horários.' 
                    : 'A enquete está oculta para os alunos enquanto você cadastra e gerencia rotas e paradas.'}
                </small>
              </div>
            </div>
            <div>
              {enqueteAberta ? (
                <button
                  type="button"
                  className="btn btn-sm btn-outline-danger fw-bold rounded-3 px-3 py-2"
                  onClick={handleEncerrarEnquete}
                >
                  Fechar / Ocultar Enquete
                </button>
              ) : (
                <button
                  type="button"
