    ...
    try {
      ...
      if (res && Array.isArray(res) && res.length > 0) {
        ...
        const mapped = res.map((v, idx) => ({
          id: v.id || idx + 1,
          numero: v.nickname || parseInt(v.id) || idx + 1,
      ...
      <div className="d-flex justify-content-between align-items-center mb-5 pb-1">
        ...
        <div className="d-flex align-items-center gap-3">
          <button 
            ...
            title="Alternar entre a visão em Grade (Figma) e em Tabela"
          >
            <FontAwesomeIcon icon={view === 'figma' ? faList : faTh} />
            <span>{view === 'figma' ? 'Ver em Tabela' : 'Grade do Figma'}</span>
          </button>
          <ProfileIcon />
        </div>
      </div>
