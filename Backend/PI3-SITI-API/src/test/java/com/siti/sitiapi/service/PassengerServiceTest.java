...
public class PassengerServiceTest {
    ...
    void testVoteUserNotFound() {
        when(userRepository.findByEmail(anyString())).thenReturn(null);
        RuntimeException exception = assertThrows(RuntimeException.class, () -> passengerService.vote("test@test.com", new com.siti.sitiapi.dto.VoteSubmitRequest()));
        assertEquals("Usuário não encontrado.", exception.getMessage());
    }
    ...
    void testVoteSuccess() {
        ...
        com.siti.sitiapi.dto.VoteSubmitRequest payload = new com.siti.sitiapi.dto.VoteSubmitRequest();
        payload.setPollId(1L);
        payload.setBoardingStopId(2L);
        payload.setBoardingScheduleId(3L);
        payload.setAlightingStopId(4L);
        payload.setAlightingScheduleId(5L);

        Map<String, Object> result = passengerService.vote("test@test.com", payload);
        assertEquals(true, result.get("success"));
        assertEquals(1L, result.get("pollId"));
        verify(jdbc, times(1)).update(anyString(), eq(1L), eq(1L), eq(2L), eq(3L), eq(4L), eq(5L));
    }
    ...
    void testGetNotices() {
        ...
        when(userRepository.findByEmail("test@test.com")).thenReturn(user);
        when(jdbc.query(anyString(), any(RowMapper.class), eq(1L))).thenReturn(new ArrayList<>());
        assertNotNull(passengerService.getNotices("test@test.com"));
    }
