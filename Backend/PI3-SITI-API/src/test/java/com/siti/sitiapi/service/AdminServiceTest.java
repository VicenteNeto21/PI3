...
public class AdminServiceTest {
    ...
    void testCreateDriverSuccess() {
        ...
        assertEquals(1L, result.get("id"));
        assertEquals("Ativo", result.get("status"));
        verify(adminRepository, times(1)).insertDriver(any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void testCreateRouteWithStops() {
        ...
        when(adminRepository.insertStop(any())).thenReturn(4L);

        Map<String, Object> result = adminService.createRoute(payload);
        assertEquals(1L, result.get("id"));
        
        verify(adminRepository, times(1)).insertStop(2L);
        verify(adminRepository, times(1)).insertRouteStop(1L, 4L, 1);
        verify(adminRepository, times(1)).insertStopSchedule(4L, 3L);
    }
