...
public class AdminRepository {
    ...
    public List<Map<String, Object>> getVehicles() {
        return jdbc.query(
                "SELECT b.id, b.nickname, b.license_plate AS plate, b.bus_model AS model, b.manufacturing_year, b.capacity, b.id_driver AS driverId, " +
                ...
                "       ) AS votes " +
                "FROM buses b " +
                "LEFT JOIN drivers d ON b.id_driver = d.id",
                (rs, rowNum) -> {
                    ...
                    map.put("driverName", dn != null ? dn : "");
                    Object driverId = rs.getObject("driverId");
                    map.put("driverId", driverId != null ? ((Number) driverId).longValue() : null);
                    return map;
                }
        );
    }
    ...
    public void assignDriverToVehicle(Long vehicleId, Long driverId) {
        jdbc.update("UPDATE buses SET id_driver = ? WHERE id = ?", driverId, vehicleId);
    }

    public void unassignDriverFromVehicle(Long vehicleId) {
        jdbc.update("UPDATE buses SET id_driver = NULL WHERE id = ?", vehicleId);
    }
    ...
    public List<Map<String, Object>> getDrivers() {
        return jdbc.query(
                ...
                (rs, rowNum) -> {
                    ...
                    map.put("cnh", rs.getString("cnh") != null ? rs.getString("cnh") : "");
                    map.put("category", rs.getString("category") != null ? rs.getString("category") : "");
                    map.put("birthDate", birthDateSql != null ? birthDateSql.toString() : "");
                    map.put("validity", validitySql != null ? validitySql.toString() : "");
                    map.put("phone", rs.getString("phone") != null ? rs.getString("phone") : "");
                    map.put("email", rs.getString("email") != null ? rs.getString("email") : "");
                    map.put("status", rs.getString("status"));
                    String cpfDoc = rs.getString("cpf");
                    map.put("cpf", (cpfDoc != null && !cpfDoc.isEmpty()) ? cpfDoc : "000.000.000-00");
                    return map;
                }
        );
    }

    public void insertDriver(Long id, String name, String phone, java.sql.Date birthDate, String cnh, String category, java.sql.Date validity, String cpf) {
        if (cpf != null && !cpf.isEmpty()) {
            jdbc.update("UPDATE users SET name = ?, identifier_document = ?, status = 'Ativo' WHERE id = ?", name, cpf, id);
        } else {
            jdbc.update("UPDATE users SET name = ?, status = 'Ativo' WHERE id = ?", name, id);
        }
        jdbc.update(
                "INSERT INTO drivers (id, name, phone, birth_date, cnh_number, cnh_category, cnh_validity_date, id_address) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, NULL) " +
                "ON DUPLICATE KEY UPDATE " +
                "name = VALUES(name), phone = VALUES(phone), birth_date = VALUES(birth_date), " +
                "cnh_number = VALUES(cnh_number), cnh_category = VALUES(cnh_category), cnh_validity_date = VALUES(cnh_validity_date)",
                id, name, phone, birthDate, cnh, category, validity
        );
    }

    public void updateDriver(Long id, String name, String phone, java.sql.Date birthDate, String cnh, String category, java.sql.Date validity, String cpf) {
        if (cpf != null && !cpf.isEmpty()) {
            jdbc.update("UPDATE users SET name = ?, identifier_document = ? WHERE id = ?", name, cpf, id);
        } else {
            jdbc.update("UPDATE users SET name = ? WHERE id = ?", name, id);
        }
        ...
        if (updated == 0) {
            insertDriver(id, name, phone, birthDate, cnh, category, validity, cpf);
        }
    }
    ...
    public void deleteStop(Long id) {
        jdbc.update("DELETE FROM poll_boarding_stops WHERE stop_id = ?", id);
        jdbc.update("DELETE FROM poll_alighting_stops WHERE stop_id = ?", id);
        jdbc.update("DELETE FROM route_stops WHERE id_stop = ?", id);
        jdbc.update("DELETE FROM stop_schedules WHERE id_stop = ?", id);
        jdbc.update("DELETE FROM stops WHERE id = ?", id);
    }
}
