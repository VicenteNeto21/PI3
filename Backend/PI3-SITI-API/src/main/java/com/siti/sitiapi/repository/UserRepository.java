...
public class UserRepository implements BaseRepository{
    ...
    public void updateUserForDriver(Long userId, String name, String identifierDocument, String password) {
        jdbc.update("UPDATE users SET name = ?, identifier_document = ?, status = 'Ativo' WHERE id = ?", name, identifierDocument, userId);
        if (password != null && !password.isBlank() && !"••••••••".equals(password)) {
            String hash = passwordEncoder.encode(password);
            jdbc.update("UPDATE users SET password = ? WHERE id = ?", hash, userId);
        }
    }
}