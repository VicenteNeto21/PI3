...
public class DriverService {
    ...
    public Map<String, Object> getProfile(String email) {
        ...
        // Buscar ônibus fixo atribuído na tabela buses
        List<Map<String, Object>> assignedBuses = jdbc.query(
                ...
                "FROM buses b WHERE b.id_driver = ? LIMIT 1",
                (rs, rowNum) -> {
                    Map<String, Object> map = new java.util.HashMap<>();
                    map.put("id", rs.getLong("id"));
        ...
        response.put("name", driver.getName() != null ? driver.getName() : "Carlos Motorista");
        response.put("cpf", user.getIdentifierDocument() != null ? user.getIdentifierDocument() : "123.456.789-01");
        response.put("cnh", driver.getCnhNumber() != null ? driver.getCnhNumber() : "12345678901");
        response.put("category", driver.getCnhCategory() != null ? driver.getCnhCategory().name() : "D");
        response.put("birthDate", driver.getBirthDate() != null ? driver.getBirthDate().toString() : "1985-06-23");
        response.put("validity", driver.getCnhValidityDate() != null ? driver.getCnhValidityDate().toString() : "2031-12-31");
        response.put("phone", driver.getPhone() != null ? driver.getPhone() : "(88) 98888-7777");
        response.put("email", email);
        response.put("status", user.getStatus() != null ? user.getStatus() : "Ativo");
        response.put("trips", trips);
        response.put("assignedVehicle", !assignedBuses.isEmpty() ? assignedBuses.get(0) : null);
        return response;
    }
    ...
    public Map<String, Object> getVehicle(String email) {
        ...
        // 1. Primeiro verifica se há ônibus permanentemente atribuído a este motorista na tabela buses
        ...
        // 2. Fallback: busca por viagem escalada do dia na tabela trips
        vehicles = jdbc.query(
                "SELECT b.id, b.nickname, b.bus_model AS model, b.license_plate AS plate, b.manufacturing_year, " +
                "       b.capacity, CASE WHEN b.accessibility = 1 THEN 'Sim (Elevador)' ELSE 'Não' END AS accessibility, " +
                "       COALESCE(b.operation_status, 'Ativo') AS status, " +
        ...
        if (vehicles.isEmpty()) {
            return Map.of(); // Retorna vazio se não tiver ônibus escalado nem fixo
        }
